...
class DataLoader(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.ix_to_word = self.info['ix_to_word'] # index and vocabulary
        self.vocab_size = len(self.ix_to_word)
        self.logger.info('vocab size is %d' %self.vocab_size)

        # open the hdf5 file
        print('DataLoader loading h5 file: ', opt.input_label_h5)
        self.h5_label_file = h5py.File(self.opt.input_label_h5, 'r', driver='core')  # discretized caption data
    ...
    def get_objectLabel(self, img_ix):
        # obj_label = [[0, 4,8,2,1,9,6],[],[],...,[]] represents words in i2w in coco_pred_sg_rela.npy, first three represents objects and se
        # second three descrides the object like: red man, object: man; red describes man
        ix = img_ix
        image_id = str(self.info['images'][ix]['id'])
        folder = 'data/coco_img_sg/'
        tmp = np.load(folder + image_id + '.npy')[()]
        obj_label = tmp['obj_attr']  # the values mean dict[] in obj_attr dictionary
        return obj_label

    def get_senTriples(self, img_ix):
        ix = img_ix
        image_id = str(self.info['images'][ix]['id'])
        folder = 'data/coco_spice_sg2/'
        tmp = np.load(folder + image_id + '.npy')[()]
        triples = tmp['rela_info']  # the values mean dict[] in obj_attr dictionary
        return triples
    ...
    def get_batch(self, split, batch_size=None, seq_per_img=None):
        ...
        for i in range(batch_size):
            ...
            obj_label = self.get_objectLabel(ix)  # labels of objects detected in an image
            sen_triples = self.get_senTriples(ix)  # triples obtained from sentences
            fc_batch.append(tmp_fc)
            att_batch.append(tmp_att)
            vrg_batch.append(tmp_vrg)
            obj_label_batch.append(obj_label)
            triples_batch.append(sen_triples)
    ...
    def get_graph_data(self, index):
        image_id = str(self.info['images'][index]['id'])  # type: string
